...
class EEGNet(Network):
    ...
    def build_model(self):  # EEGNet
        self.model = keras.Sequential([
            ...
            Dropout(self.p),  # default 0.25: cross-subject classification
                              # (change to 0.5 if within-subject classification)
...
def run_cv_EEGNet(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        with open(f'outputs/y_test_EEGNet_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_EEGNet_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_EEGNet_{fold}_hpc.txt', 'w') as f2:
...
with open(f'outputs/runtime_EEGNet_hpc.txt', 'w') as f3:
