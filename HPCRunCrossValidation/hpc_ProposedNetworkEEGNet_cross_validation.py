...
from keras.layers import Dense, Conv2D, DepthwiseConv2D, Activation, Input, SeparableConv2D
...
def run_cv_ProposedNetworkEEGNet(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        network = ProposedNetworkEEGNet(ep=ep)
        ...
        with open(f'outputs/y_test_ProposedNetworkEEGNet_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_ProposedNetworkEEGNet_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_ProposedNetworkEEGNet_{fold}_hpc.txt', 'w') as f2:
...
run_cv_ProposedNetworkEEGNet(5, 10)

end_time = time.time()
end_time_cpu = time.process_time()

with open(f'outputs/runtime_ProposedNetworkEEGNet_hpc.txt', 'w') as f3:
