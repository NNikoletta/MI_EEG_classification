...
from keras.layers import Dense, Conv2D, DepthwiseConv2D, Activation, Input
from keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, LSTM
from keras.layers import Flatten, Concatenate, Multiply
...
def run_cv_ProposedNetworkLSTM(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        network = ProposedNetworkLSTM(ep=ep)
        ...
        with open(f'outputs/y_test_ProposedNetworkLSTM_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_ProposedNetworkLSTM_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_ProposedNetworkLSTM_{fold}_hpc.txt', 'w') as f2:
...
run_cv_ProposedNetworkLSTM(5, 10)

end_time = time.time()
end_time_cpu = time.process_time()

with open(f'outputs/runtime_ProposedNetworkLSTM_hpc.txt', 'w') as f3:
