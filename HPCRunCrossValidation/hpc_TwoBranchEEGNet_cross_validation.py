...
from keras.layers import AveragePooling2D, Activation, Concatenate, BatchNormalization
from keras.layers import SeparableConv2D, DepthwiseConv2D, Conv2D
...
def run_cv_TwoBranchEEGNet(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        with open(f'outputs/y_test_TwoBranchEEGNet_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_TwoBranchEEGNet_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_TwoBranchEEGNet_{fold}_hpc.txt', 'w') as f2:
...
run_cv_TwoBranchEEGNet(5, 10)

end_time = time.time()
end_time_cpu = time.process_time()

with open(f'outputs/runtime_TwoBranchEEGNet_hpc.txt', 'w') as f3:
