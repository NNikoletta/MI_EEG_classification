...
def run_cv_DSCNN(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        with open(f'outputs/y_test_DSCNN_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_DSCNN_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_DSCNN_{fold}_hpc.txt', 'w') as f2:
            f2.write("Test loss: " + str(test_loss) + " Test acc: " + str(test_acc) + '\n')
        f2.close()

        fname = f'outputs/DSCNN_{fold}_hpc'
        plot_accuracy_and_loss(history, 'DSCNN', True, fname)
...
with open(f'outputs/runtime_DSCNN_hpc.txt', 'w') as f3:
