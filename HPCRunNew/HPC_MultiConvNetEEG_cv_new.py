...
def run_cv_MultiConvNetEEG(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        network = net.ProposedNetworkEEGNet(ep=ep)
        ...
        with open(f'outputs/y_test_MultiConvNetEEG_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_MultiConvNetEEG_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_MultiConvNetEEG_{fold}_hpc.txt', 'w') as f2:
            f2.write("Test loss: " + str(test_loss) + " Test acc: " + str(test_acc) + '\n')
        f2.close()

        fname = f'outputs/MultiConvNetEEG_{fold}_hpc'
        plot_accuracy_and_loss(history, 'MultiConvNetEEG', True, fname)
...
run_cv_MultiConvNetEEG(10, 10)

end_time = time.time()
end_time_cpu = time.process_time()

with open(f'outputs/runtime_MultiConvNetEEG_hpc.txt', 'w') as f3:
