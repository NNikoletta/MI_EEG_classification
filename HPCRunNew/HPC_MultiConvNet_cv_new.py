...
def run_cv_MultiConvNet(n_folds=5, ep=10):
    ...
    for i in range(n_folds):
        ...
        with open(f'outputs/y_test_MultiConvNet_{fold}_hpc.txt', 'w') as f:
            for k in range(len(y_test)):
                f.write(str(y_test[k]) + '\n')
        f.close()

        with open(f'outputs/predicted_classes_MultiConvNet_{fold}_hpc.txt', 'w') as f1:
            for k in range(len(predicted_classes)):
                f1.write(str(predicted_classes[k]) + '\n')
        f1.close()

        with open(f'outputs/test_results_MultiConvNet_{fold}_hpc.txt', 'w') as f2:
            f2.write("Test loss: " + str(test_loss) + " Test acc: " + str(test_acc) + '\n')
        f2.close()

        fname = f'outputs/MultiConvNet_{fold}_hpc'
        plot_accuracy_and_loss(history, 'MultiConvNet', True, fname)
...
run_cv_MultiConvNet(10, 10)

end_time = time.time()
end_time_cpu = time.process_time()

with open(f'outputs/runtime_MultiConvNet_hpc.txt', 'w') as f3:
